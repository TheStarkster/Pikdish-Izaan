...
class RestaurantListing extends Component {
  ...
  async componentDidMount() {
    try {
      ...
      await Promise.all([
        this.fetchData(),
        this.fetchTopAds(),
        this.fetchRestroDisAds(),
        this.fetchMiddleVegAds(),
        this.fetchMiddleCuisinesAds(),
        this.fetchRestroUnder100Ad(),
        this.fetchRestroFiftyPerOffAd(),
        this.fetchRestroNewRestroAds(),
        this.fetchRestroBrandAd(),
      ]);
    } catch (e) {
  ...
  renderItems = () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (i === 0) {
        mainContainerStyles.marginTop = 0;
      }
